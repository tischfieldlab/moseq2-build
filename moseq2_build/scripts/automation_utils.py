...
from moseq2_build.utils.constants import DEFAULT_FLIP_PATH, BATCH_TABLE, EXTRACT_TABLE, \
    SINGULARITY_COMS, ENVIRONMENT_CONFIG, getDefaultImage
...
def main():
    ...
    for _, subp in subparsers.choices.items():
        subp.add_argument('-p', '--p', default=getDefaultImage(), nargs='?', help='Location of the image file to be used.', dest='imagePath')
...
def entrypoint(args):
    ...
    if (args.imagePath.endswith('.sif')):
        print(colored("\nDetected singularity image at {}\n".format(os.path.abspath(args.imagePath)),
            'white', attrs=['bold']))
